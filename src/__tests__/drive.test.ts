...
describe("local driver lifecycle", () => {
  ...
  it("fails the lane on a 500 from its state endpoint and keeps the process going", async () => {
    type PolledState = {
      runId: string;
      observedAt: string;
    ...
    const stopBody = JSON.stringify({
      ...
      artifacts: [],
      shutdown: {
        destroy: { attempted: true, acknowledged: true, error: null },
      },
    });
    const upstream = vi.fn((input: RequestInfo | URL) => {
      if (String(input).endsWith("/state")) {
        return Promise.resolve(
          new Response("the control plane exploded", { status: 500 }),
        );
      }
      return Promise.resolve(new Response(stopBody, { status: 200 }));
    });
    vi.stubGlobal("fetch", upstream);
    ...
    const outcome = await runWithCleanup(
      () =>
        driveUntilComplete({
          ...
          requested: { head: "a".repeat(40), base: "b".repeat(40) },
          role: "reviewer",
          laneId: "lane-1",
          candidateIds: [],
          deadline: 1_000,
    ...
    expect(String(upstream.mock.calls[0]?.[0])).toContain("/runs/lane-1/state");
    expect(outcome.runError).toEqual(
      expect.objectContaining({
        message: expect.stringContaining("/runs/lane-1/state: 500"),
      }),
    );
    expect(cleanup).toHaveBeenCalledOnce();
    expect(outcome.cleanupResult).toMatchObject({ runId: "lane-1" });
